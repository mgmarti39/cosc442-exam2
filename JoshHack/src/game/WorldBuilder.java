...
public class WorldBuilder {
	...
	private void smooth_2(Tile[][][] tiles2, int x, int y, int z) {
		int floors = 0;
		int rocks = 0;

		for (int ox = -1; ox < 2; ox++) {
			for (int oy = -1; oy < 2; oy++) {
				if (smooth_3(x, y, ox, oy))
					continue;
	...
	private boolean smooth_3(int x, int y, int ox, int oy) {
		return x + ox < 0 || x + ox >= width || y + oy < 0
				|| y + oy >= height;
	}
	...
	private void createRegions_2(int z, int x, int y) {
	...
	private int fillRegion(int region, int x, int y, int z) {
		...
		while (!open.isEmpty()){
			...
			for (Point neighbor : p.neighbors8()){
				size = fillRegion_2(region, size, open, neighbor);
			}
		}
		return size;
	}

	private int fillRegion_2(int region, int size, ArrayList<Point> open, Point neighbor) {
		if (neighbor.x < 0 || neighbor.y < 0 || neighbor.x >= width || neighbor.y >= height)
			return size;
		
		if (fillRegion_3(neighbor))
			return size;
	...
	private boolean fillRegion_3(Point neighbor) {
		return regions[neighbor.x][neighbor.y][neighbor.z] > 0
	...
	public List<Point> findRegionOverlaps(int z, int r1, int r2) {
		...
		for (int x = 0; x < width; x++){
			for (int y = 0; y < height; y++){
				findRegionOverlaps_2(z, r1, r2, candidates, x, y);
			}
		}
	...
	private void findRegionOverlaps_2(int z, int r1, int r2, ArrayList<Point> candidates, int x, int y) {
		if (tiles[x][y][z] == Tile.FLOOR
				&& tiles[x][y][z+1] == Tile.FLOOR
				&& regions[x][y][z] == r1 
				&& regions[x][y][z+1] == r2){
